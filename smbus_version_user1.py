...
class smbusControl:
    def __init__(self, serverport, serverip, clientport, clientip, gpio_port, address, gpiodirection):
        self.gpio = gpio_port
    ...
    def gpio_to_ip(self):
        self.socket = self.launchServer()
        packedFmt   = struct.Struct('I')
        while self.alive:
            try:
                data = self.readGpio()            # read one, blocking
                #print >>sys.stderr, 'reading smbus', data
                sendData = packedFmt.pack(data)
                self._write_lock.acquire()
                try:
                    self.socket.sendall(sendData)           # send it over TCP
                finally:
                    self._write_lock.release()
            except socket.error, msg:
                sys.stderr.write('ERROR: %s\n' % msg)
    ...
    def ip_to_gpio(self):
        ...
        while self.alive:
            try:
                ...
                self.writeGpio(recvData[0])                 # get a bunch of bytes and send them
                # the spy shows what's on the serial port, so log it after converting newlines
            except socket.error, msg:
                sys.stderr.write('ERROR: %s\n' % msg)
                # probably got disconnected
